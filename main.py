...
obj = LinearRegression(x_train,y_train, 0.04, 500)
costs = obj.run_linear_regression()


y_predict_train = obj.predict(x_train)
R2_sklearn_train = r2_score(y_train, y_predict_train)
print(f"R² Linear Regression Train Data (Scikit-Learn Calculation): {R2_sklearn_train}")


y_predict_test = obj.predict(x_test)
R2_sklearn = r2_score(y_test, y_predict_test)
print(f"R² Linear Regression test Data (Scikit-Learn Calculation): {R2_sklearn}")
